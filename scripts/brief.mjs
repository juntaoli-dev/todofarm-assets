 ...
 * Run: npm run brief <slot-id>        one slot, full brief
 *      npm run brief                  every slot, one line each
...
const hits = q ? slots.filter(s => s.id.toLowerCase().includes(q)) : slots;
if (!hits.length) { console.error(`no slot matches "${q}". Run: npm run brief`); process.exit(1); }
